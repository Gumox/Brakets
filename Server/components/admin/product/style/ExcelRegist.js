...
const ExcelRegist =({user})=>{
    ...
    const registProduct = async() =>{
        const data = {headquarterId:user.headquarter_id ,list:excelToJson}
        
        const [result] = await Promise.all([
            axios
              .post(`${process.env.API_URL}/product/addStyleToExcel`,data)
              .then(({ data }) => data.body), 
            ])
        alert("스타일이 등록되었습니다.")
    ...
    return(
        <Wrapper>
            <InsideWrapper>
            <div style={{marginBottom:20,fontSize:20,fontWeight:"bold"}}>스타일 엑셀 업로드</div>
