...
const SearchFocus = ({
  ...
  return(
      <div 
        style={{display:"flex",width:"346px",minHeight:"60px",position:"absolute",top:2,flexDirection:"column"}}
      ...
      >
        <InputLine placeholder="검색 or 입력" value={name || ""} style={{flex:1,minHeight:"58px",border:0,padding:10 ,display:"flex",textAlign:"center",}} 
              ...
              {shopListResult.length>0 &&
...

                {
                  shopListResult.map((item,index)=>(
                    <SearchResult key={index}  style={{width:"340x",height:"45px",cursor:"pointer"}} onClick={()=>{clicker(item.store_id,item.name)}} >{item.name}</SearchResult>
                  ))
                }
