    ...
    <Wrapper style={{height:`${windowHeight}px`}}>
      ...
      <InSideWrapper>
        <SidebarSpace  style={{minHeight:`${windowHeight-120}px`}}>
          <PlatformSideBar path={"/admin/platformControl/controlRepairTypeList"}/>
        </SidebarSpace>
        <MainSpace >
          <RepairTypeListControl user={user} repairTypes={repairTypes} brands={brands}/>
        </MainSpace>
      </InSideWrapper>
    </Wrapper>
  );
...
export const getServerSideProps = async (ctx) => {
    const {
      ...
      const [repairTypes] = await Promise.all([
        axios
          .get(`${process.env.API_URL}//type/repairTypeInfo?headquarterId=${user.headquarter_id}`,)
          .then(({ data }) => data.data), 
      ])
