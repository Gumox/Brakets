...
import StaffList from "../../../components/admin/repair/staffList.js";

const RepairControlRepairShopStaff = ({user,infos,staffs,repairShop}) => {
    ...
    <Wrapper style={{height:`${windowHeight}px`}}>
      ...
      <InSideWrapper>
        <SidebarSpace  style={{minHeight:`${windowHeight-150}px`}}>
          <RepairSideBar path={"/admin/repairControl/repairShopStaff"}/>
        </SidebarSpace>
        <MainSpace >
          <StaffList repairShopStaff={staffs}/>
        </MainSpace>
      </InSideWrapper>
    </Wrapper>
  );
...
export const getServerSideProps = async (ctx) => {
  ...
  if (!isAuthorized) {
    ...
    const [staffs] = await Promise.all([
      axios
        .get(`${process.env.API_URL}/RepairShop/getRepairShopStaff?headquarterId=${user.headquarter_id}`,)
        .then(({ data }) => data.data), 
    ])
...
export default RepairControlRepairShopStaff;
