...
async function returnInvoiceLog(
    ...
    const result = await excuteQuery({
        query: 
        "INSERT INTO" + 
        "`invoice_claim_store`(`receipt_id`, `receipt_code`, `release_date`, `status`, `season`, `partcode`, `color`, `size`, `qty`, `amount`, `created`, `created_date`, `edited`, `edited_date` )" + 
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
async function getInvoice(receipt_code){
  const result = await excuteQuery({
    query: 
            `SELECT * FROM invoice_claim_store WHERE receipt_id =?`,
    values:[receipt_code],
  });
  return result
}
